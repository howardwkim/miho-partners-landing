 ...
 */
export async function BlogSampler({ padding = "py-20" }: { padding?: string }) {
  const posts = (await getAllPosts()).slice(0, 3);
  if (posts.length === 0) return null;

  return (
    <section className={`mx-auto w-full max-w-5xl px-6 sm:px-10 ${padding}`}>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <h2 className="text-3xl font-light tracking-tight sm:text-4xl">How we think about it</h2>
        <Link
          href="/blog"
